...
export default function BlingIntegrationScreen() {
  ...
  return <SafeAreaView style={styles.root} edges={["top"]}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Text style={styles.subtitle}>Consulte propostas comerciais e seus detalhes. A importação só ocorre após sua confirmação explícita. Pedidos no Bling só são criados após confirmação na proposta aprovada.</Text>
    {loading ? <ActivityIndicator color={theme.colors.primary} /> : <View style={styles.card}>
      ...
      {status?.connected && <View style={styles.connectionActions}>
        <Pressable accessibilityRole="button" style={[styles.secondaryButton, (connecting || previewing || fetching) && styles.disabled]} disabled={connecting || previewing || fetching} onPress={connect}>{connecting ? <ActivityIndicator color={theme.colors.primary} /> : <Text style={styles.secondaryButtonText}>Reconectar Bling</Text>}</Pressable>
        <Pressable accessibilityRole="button" style={[styles.button, (connecting || previewing || fetching) && styles.disabled]} disabled={connecting || previewing || fetching} onPress={preview}>{previewing ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Ver prévia de propostas</Text>}</Pressable>
      </View>}
    </View>}
