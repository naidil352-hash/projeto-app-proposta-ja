...
async def update_outbound_webhook(webhook_id: str, data: OutboundWebhookUpdateIn, user=Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(422, str(exc)) from exc
    if not changes:
        raise HTTPException(422, "Informe uma altera+�+�o")
...
@api_router.get("/integrations/bling/commercial-proposals/{proposal_id}")
async def preview_bling_commercial_proposal_detail(proposal_id: str, user: dict = Depends(get_current_user)):
    company_id = _safe_company_id(user)
